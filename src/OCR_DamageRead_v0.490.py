...
import numpy as np
import pytesseract
from PyQt5.QtWidgets import (QApplication, QWidget, QLabel, QVBoxLayout, QHBoxLayout, QRadioButton, QButtonGroup, QPushButton, QLineEdit)
from PyQt5.QtGui import QImage, QPixmap, QColor, QPalette, QIntValidator
from PyQt5.QtCore import Qt

# Pylance の型スタブが Qt 名前空間の属性を直接解決できない場合があるため、AlignmentFlag / TextFormat の定数をローカル変数として取り出す
# 実行時の挙動は変わらない（同じ整数値）。
_AlignTop    = Qt.AlignTop       # type: ignore[attr-defined]
_AlignCenter = Qt.AlignCenter    # type: ignore[attr-defined]
_RichText    = Qt.RichText       # type: ignore[attr-defined]
...
def _find_tesseract() -> str:
    """
    Tesseract の実行ファイルを探して返す。
    1. 環境変数 TESSERACT_CMD が設定されていればそれを使う
    2. Windows のデフォルトインストールパスを確認
    3. PATH 上に tesseract があればそれを使う（Linux/Mac）
    ...
    raise RuntimeError(
        "Tesseract が見つかりません。\n"
        "インストール後、環境変数 TESSERACT_CMD に実行ファイルのパスを設定してください。\n"
        "例: set TESSERACT_CMD=C:\\Program Files\\Tesseract-OCR\\tesseract.exe"
    )

pytesseract.pytesseract.tesseract_cmd = _find_tesseract()
# pyファイルがある src/ ディレクトリを基準パスとして使う
_HERE = os.path.dirname(os.path.abspath(__file__))

# =========================================================
# 初期設定クラス：　16:9フルスクリーンでのチューニング
# =========================================================
@dataclass
class Config:
...
#
# 設定の目安：
#   80  → word conf が低い場合のみ再確認。psm10 発動は稀で、偶に誤検出を見逃すが速い
#   85  → 中間。2000サンプルほどで誤検出した最高conf値が84だったためこの値とした。高品質な画像では発動頻度低下
#   90  → 旧実装に近い挙動（発動頻度は高め）
# -----------------------------------------------------------
CONF_WORD_THRESHOLD: float = 85.0

# デバッグユーティリティ
# スロット並列処理用スレッドローカルストレージ（デバッグ出力の競合を防ぐ）
...
def _dbg(msg: str):
    """デバッグメッセージをスレッドローカルバッファに追記する。CONFIG.debug_mode が False の場合は何もしない。"""
...
class OCRResult:
    """OCR結果を格納"""
    value: int                  # 読み取った数値
    ocr_str: str                # OCR が返した生文字列
    ocr_len: int                # ocr_str の文字数
    digit_count: int            # CC解析で検出した桁数
    digit_ok: bool              # digit_count と ocr_len が一致しているか
    confidence: float           # 全桁の信頼度平均（表示・後方互換用）
    char_confs: List[float]     # 桁ごとの信頼度リスト（ocr_str と同順）
    error_code: Optional[str]   # エラーコード
    cc_info: list               # CC解析結果（bbox・種別等）。digit_bin_vis への矩形描画に使用
    digit_bin_vis: np.ndarray   # CC解析対象の2値画像。デバッグ表示でcc_infoの矩形を重ねて描画する
    ocr_img: np.ndarray         # OCRに渡した加工済み画像（マージン追加・太字化済み）。デバッグ表示用
    band_vis: np.ndarray        # 数字帯のカラープレビュー画像。デバッグウィンドウの先頭に表示する
...
class OCRProcessor:
    ...
    @classmethod
    def _extract_digit_band(
        cls, bin_raw: np.ndarray
    ) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray, list, list, int, np.ndarray]]:
        """
        【ステップ1】数字帯の検出・CC解析・ノイズ除去を行う。

        Returns
        -------
        None
            数字帯が見つからない場合。
        (band_vis, cc_src, digit_only, ccs, digit_ccs, digit_count, ocr_img)
            正常に数字帯が確定できた場合。
        """
    ...
    def _run_psm7(
        ...
        return ocr_str, ocr_len, confidence, char_confs

    @classmethod
    def _recover_missing_digits_pipeline(
        cls,
        digit_only:  np.ndarray,
        digit_ccs:   list,
        ocr_str:     str,
        ocr_len:     int,
        digit_count: int,
        ...
        """
        【ステップ3】桁欠損リカバリー（digit_count > ocr_len のとき発動）。

        psm10 で各桁を個別認識し、欠損桁を補完する。

        Returns
        -------
        (ocr_str, ocr_len, error_code, recover_digits_attempted)
        """
        if not (digit_count > ocr_len > 0):
            return ocr_str, ocr_len, None, False

        missing = cls.recover_missing_digits(
            digit_only, digit_ccs, ocr_str, digit_count, slot_idx=-1
        )

        if missing is not None:
            # psm10 で全桁を再認識し、「?」はpsm7結果で補完する
            psm10_full = []
            for cc in digit_ccs:
                x, y, w, h = cc["bbox"]
            ...
            if ocr_str.isdigit() and "?" not in ocr_str:
                return ocr_str, ocr_len, "RECOVER_DIGITS", True
            else:
                return ocr_str, ocr_len, "RECOVER_FAILED", True
        else:
            return ocr_str, ocr_len, "RECOVER_FAILED", True

    @classmethod
    def _recover_value_mismatch_pipeline(
        cls,
        ocr_str:     str,
        digit_only:  np.ndarray,
        digit_ccs:   list,
        ...
        """
        【ステップ4】値誤検出リカバリー（5→6/8/9 の誤認識を修正）。

        word conf が低い桁の 6/8/9 を psm10 で再確認し、
        5 だった場合に修正する。

        Returns
        -------
        (ocr_str, error_code_or_None, recover_value_attempted)
            error_code_or_None は修正が発生した場合のみ "RECOVER_VALUE_MISMATCH"。
        """
        if not any(d in ocr_str for d in ("6", "8", "9")):
            return ocr_str, None, False
        ...
        if new_str != ocr_str and new_str.isdigit():
            return new_str, "RECOVER_VALUE_MISMATCH", recover_value_attempted

        return ocr_str, None, recover_value_attempted

    @classmethod
    def process_roi(cls, img: np.ndarray) -> OCRResult:
        """
        ROI 画像から数値を読み取り OCRResult を返す。

        処理パイプライン:
          1. _extract_digit_band()              数字帯検出・CC解析
          2. _run_psm7()                        psm7 OCR
          3. _recover_missing_digits_pipeline() 桁欠損リカバリー
          4. 通常エラー判定
          5. _recover_value_mismatch_pipeline() 値誤検出リカバリー
        """
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        _, bin_raw = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY_INV)

        # ---- ステップ1: 数字帯検出・CC解析 ----
        band_result = cls._extract_digit_band(bin_raw)
        if band_result is None:
            return OCRResult(
                value=0, ocr_str="", ocr_len=0,
                digit_count=0, digit_ok=False,
                confidence=0.0, char_confs=[],
                error_code="NO_DIGIT", cc_info=[],
                digit_bin_vis=np.zeros_like(bin_raw),
                ocr_img=np.zeros((1, 1), dtype=np.uint8),
                band_vis=cv2.cvtColor(
                    np.zeros((CONFIG.digit_band_height, bin_raw.shape[1]), dtype=np.uint8),
                    cv2.COLOR_GRAY2BGR,
                ),
        ...
        band_vis, cc_src, digit_only, ccs, digit_ccs, digit_count, ocr_img = band_result
        digit_ok = 1 <= digit_count <= 8

        # ---- ステップ2: psm7 OCR ----
        ocr_str, ocr_len, confidence, char_confs = cls._run_psm7(ocr_img)
        ...
        if digit_ok:
            ocr_str, ocr_len, error_code, recover_digits_attempted = (
                cls._recover_missing_digits_pipeline(
                    digit_only, digit_ccs, ocr_str, ocr_len, digit_count
                )
            )
        ...
        if error_code is None:
            ...
            # リカバリー済みの場合は error_code が確定しているので value だけ設定
            if error_code in ("RECOVER_DIGITS",):
                value = int(ocr_str) if ocr_str.isdigit() else 0
            # RECOVER_FAILED は value=0 のまま

        # ---- ステップ5: 値誤検出リカバリー ----
        if error_code == "OCR_OK" and digit_ok and len(ocr_str) == digit_count:
            ocr_str, vm_error, recover_value_attempted = (
                cls._recover_value_mismatch_pipeline(
                    ocr_str, digit_only, digit_ccs, digit_count, char_confs
                )
            )
...
class FileManager:
    ...
    def save_incorrect_image(src_path: str, category: str, gt_line: Optional[str] = None):
        """エラー画像を保存"""
        dst_dir = os.path.join(_HERE, "image", "incorrect", category)
...
class DebugWindow(QWidget):
    ...
    def _create_slot_ui(self, idx: int, res: OCRResult, err: str) -> QVBoxLayout:
        ...
        edit.setAlignment(_AlignCenter)
        ...
        err_lbl.setAlignment(_AlignCenter)
        ...
        conf_lbl.setAlignment(_AlignCenter)
        ...
        if res.char_confs and res.ocr_str:
            ...
            char_conf_lbl.setAlignment(_AlignCenter)
            char_conf_lbl.setTextFormat(_RichText)
        ...
        info.setAlignment(_AlignCenter)
        vbox.addWidget(info)
...
# スレッドプール（通常モード専用・初回呼び出し時に生成して使い回す）
# debug_mode=True の場合は _get_slot_pool() 自体が呼ばれないため影響なし。
# このスクリプトは毎回 python コマンドで起動するため、
# モード切り替えによるプール残留は実運用上問題にならない。
...
def process_image(img_path: str) -> Tuple[List[int], List["OCRResult"]]:
    ...
    通常モード: スレッドプールで12スロットを並列処理。
    デバッグモード: メインスレッドで逐次処理。
        → _dbg() のバッファがスロットごとに確実にフラッシュされ、
          CUI のデバッグ出力がターミナルに表示される。
    """
    img = cv2.imread(img_path)
    if img is None:
        raise ValueError(f"画像読み込み失敗: {img_path}")  # main() 側でキャッチする

    H, W, _ = img.shape
    slots: List[float] = CONFIG.slots or []

    tasks: List[Tuple[int, float, np.ndarray]] = []
    for slot_idx, sx in enumerate(slots):
        cx  = int(sx * W)
        roi = img[CONFIG.y_min:CONFIG.y_max,
                  cx - CONFIG.roi_width // 2: cx + CONFIG.roi_width // 2]
        tasks.append((slot_idx, sx, roi))

    results_list: List[Optional["OCRResult"]] = [None] * len(slots)

    if CONFIG.debug_mode:
        # デバッグ時はメインスレッドで逐次処理
        # → _dbg() バッファがスロットごとにフラッシュされCUI出力が確実に出る
        for args in tasks:
            slot_idx, res = _process_slot(args)
            results_list[slot_idx] = res
    else:
        pool = _get_slot_pool()
    ...
    values = [r.value for r in final_results]
    return values, final_results
...
def main():
    """メイン処理"""
    image_dir = os.path.join(_HERE, "image")
    # 拡張子の大文字小文字を正規化してからソート（OS間の順序差異を防ぐ）
    _all_images = (
    ...
    # 重複除去（大文字小文字を区別しないFSで同一ファイルが2件マッチする場合）
    files = sorted(set(_all_images), key=lambda p: os.path.basename(p).lower())
    total = len(files)
    ...
    # Qt初期化
    app: Optional[QApplication] = QApplication(sys.argv) if CONFIG.debug_mode else None
    ...
    for idx, img_path in enumerate(files):
        ...
        try:
            ...
            print(f"[SKIP] {e}")
            output_lines.append("")   # 行ズレ防止のため空行を挿入
            continue
        ocr_errors: List[Optional[str]] = [r.error_code for r in results]
        
        # エラー分析
        if CONFIG.gt_mode and gt_lines is not None:
            gt_vals = parse_gt_line(gt_lines[idx])
            analysis = ErrorAnalyzer.compare_with_ground_truth(
                values,
                [e or "" for e in ocr_errors],   # Optional[str] → str
                gt_vals,
                results,
            )
            ...
            analysis = ErrorAnalyzer.analyze_ocr_errors(
                [e or "" for e in ocr_errors]     # Optional[str] → str
            )
        
        # デバッグモード
        if CONFIG.debug_mode and app is not None:
        ...
        if analysis.category is not None:
            gt_line = gt_lines[idx] if (CONFIG.gt_mode and gt_lines is not None) else None
            FileManager.save_incorrect_image(img_path, analysis.category, gt_line)
    
    # 結果出力
    if CONFIG.output_txt:
        result_path = os.path.join(_HERE, "result.txt")
        with open(result_path, "w", encoding="utf-8") as f:
            f.write("\n".join(output_lines))
    
    # GT統計
    if CONFIG.gt_mode and gt_lines is not None:
        _gt_slots: List[float] = CONFIG.slots or []
        total_slots = total * len(_gt_slots)
